...
class AddRatingView(object):
    ...
    def too_many_votes_from_ip_response(self, request, context):
        response = HttpResponse(
            _('Too many votes from this IP address for this object.'))
        return response
...
class AddRatingFromModel(AddRatingView):
    def __call__(self, request, model, app_label, object_id, field_name, score):
        ...
        try:
            ...
            raise Http404(_('Invalid `model` or `app_label`.'))

        return super(AddRatingFromModel, self).__call__(request,
                                                        content_type.id,
                                                        object_id,
                                                        field_name,
                                                        score)
