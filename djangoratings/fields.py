...
class RatingManager(object):
    ...
    def add(self, score, user, ip_address, cookies={}, commit=True):
        ...
        try:
            ...
            raise InvalidRating(_("%(score)s is not a valid choice for %(field_name)s")
                                    % {'score': score, 'field_name': self.field.name})

        delete = (score == 0)
        if delete and not self.field.allow_delete:
            raise CannotDeleteVote(_("you are not allowed to delete votes for %(field_name)s")
        ...
        try:
            ...
            if delete:
                raise CannotDeleteVote(_("attempt to find and delete your vote for %(field_name)s is failed")
                    % (self.field.name,))
...
class RatingCreator(object):
    ...
    def __set__(self, instance, value):
        if isinstance(value, Rating):
            ...
            raise TypeError(_("%s value must be a Rating instance, not '%r'") % (self.field.name, value))
...
class RatingField(IntegerField):
    ...
    def __init__(self, *args, **kwargs):
        if 'choices' in kwargs:
            raise TypeError(_("%s invalid attribute 'choices'") % (self.__class__.__name__,))
